# -*- coding: utf-8 -*-
"""
재고 및 가격 동기화 스크립트 (트렌드메카 버전)

바이마에 등록된 상품의 재고와 가격을 트렌드메카(trendmecca.co.kr)에서 재수집하여
ace 테이블을 업데이트하고 바이마 API로 상품을 수정합니다.

처리 흐름:
1. ace_products에서 바이마 등록 상품 조회 (is_published=1, buyma_product_id 있음)
2. 트렌드메카 HTML 재방문 → 현재 가격/재고 수집
...
사용법:
    python stock_price_synchronizer_trendmecca.py                    # 전체 실행
    python stock_price_synchronizer_trendmecca.py --brand BURBERRY   # 특정 브랜드만
    python stock_price_synchronizer_trendmecca.py --limit 100        # 최대 100개만
    python stock_price_synchronizer_trendmecca.py --dry-run          # 테스트 (API 호출 안함)
    python stock_price_synchronizer_trendmecca.py --force            # 변경 없어도 강제 API 호출
...
# 딜레이 설정
REQUEST_DELAY_MIN = 1.0  # 트렌드메카 요청 간 최소 딜레이
REQUEST_DELAY_MAX = 2.0  # 트렌드메카 요청 간 최대 딜레이
...
def extract_size_from_option_value(option_value: str) -> str:
    """option_value에서 사이즈 추출: 'USW247 730 0951 (M)' → 'M'"""
...
class StockPriceSynchronizer:
    ...
    def _create_new_session(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            main_response = self.mall_session.get(f'{BASE_URL}/index_time.html', timeout=15)
    ...
    # -------------------------------------------------
    # 2. 트렌드메카에서 가격/재고 수집 (HTML 스크래핑)
    # -------------------------------------------------
    def collect_from_trendmecca(self, product_url: str) -> Tuple[Dict, Optional[str]]:
        """
        트렌드메카 HTML 스크래핑으로 가격/재고 수집

        - product_url: source_product_url (예: https://trendmecca.co.kr/product/{slug}/{product_no}/category/{cate_no}/display/1/)
        - option_stock_data JS 변수에서 사이즈별 재고 추출
        """
        # 페이지 가져오기
        html, error = self._fetch_product_page(product_url)
        if error:
            if "403" in error or "차단" in error:
                return {}, f"상품 접근 차단: {error}"
            if "타임아웃" in error:
                return {}, f"일시적 오류 (스킵): {error}"
            return {}, error
        ...
        # 404 / 삭제된 상품 감지
        if '존재하지 않는 상품' in html or '삭제된 상품' in html:
            return {}, "상품 삭제됨 (404)"
        ...
        try:
            ...
            # sale_price가 0이면 original_price 사용
            if result['sale_price'] == 0 and result['original_price'] > 0:
                result['sale_price'] = result['original_price']

            # original_price가 0이면 sale_price 사용
            if result['original_price'] == 0 and result['sale_price'] > 0:
            ...
            # 판매 상태 확인
            if '품절' in html and 'soldout' in html.lower():
                soup = BeautifulSoup(html, 'html.parser')
                soldout_div = soup.find('div', class_='soldout')
                if soldout_div and 'SOLD OUT' in soldout_div.get_text():
                    return {}, "판매 종료 상품"

            options = []

            # PRIMARY: option_stock_data JS 변수에서 옵션 추출
            ...
            # FALLBACK: select 태그에서 옵션 추출
            if not options:
                soup = BeautifulSoup(html, 'html.parser')
                select_tag = soup.find('select', attrs={'option_title': True}) or soup.find('select', id='product_option_id1')
                if select_tag:
                    for option_tag in select_tag.find_all('option'):
                        ...
                        is_disabled = option_tag.get('disabled') is not None
                        is_soldout = '품절' in option_text

                        tag_size = extract_size_from_option_value(option_text.replace('[품절]', '').replace('품절', '').strip())
            ...
            return {}, f"option_stock_data JSON 파싱 오류: {str(e)}"
        except Exception as e:
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
    ...
    def process_single_product(self, product: Dict, idx: int, total: int, dry_run: bool, force: bool,
        ...
        try:
            # 1. 트렌드메카 HTML로 가격/재고 수집
            mall_data, error = self.collect_from_trendmecca(product['source_product_url'])
            if error:
                add_log(f"  트렌드메카 수집 실패: {error}", "WARNING")
...
def main():
    parser = argparse.ArgumentParser(description='바이마 재고/가격 동기화 (trendmecca)')

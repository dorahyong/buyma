...
class RawToAceConverter:
    ...
    def __init__(self, db_url: str):
        ...
        self._mall_sites_cache = {}  # 수집처별 설정 캐시 (mall_sites)
    ...
    def load_mall_sites(self) -> Dict[str, Dict]:
        """수집처별 설정 로드 (mall_sites)"""
        if self._mall_sites_cache:
            return self._mall_sites_cache

        with self.engine.connect() as conn:
            result = conn.execute(text("""
                SELECT site_name, has_own_images
                FROM mall_sites
                WHERE is_active = 1
            """))
            for row in result:
                self._mall_sites_cache[row[0]] = {
                    'has_own_images': row[1]
                }

        log(f"수집처 설정 {len(self._mall_sites_cache)}건 로드 완료")
        return self._mall_sites_cache

    def has_own_images(self, source_site: str) -> bool:
        """해당 수집처가 자체 이미지를 제공하는지 확인"""
        mall_sites = self.load_mall_sites()
        return mall_sites.get(source_site, {}).get('has_own_images', 0) == 1
    ...
    def get_brand_info(self, brand_en: str) -> Dict:
        ...
        if key in brand_mapping:
            ...
            # brand_id=0 (바이마 미등록 브랜드)이면 mall_brands의 buyma_brand_name 사용
            # buyma_brand_name이 NULL인 경우에만 수집처 brand_name_en 폴백
            if not result.get('buyma_brand_id') and not result.get('buyma_brand_name'):
                result['buyma_brand_name'] = brand_en
            return result
        return {'buyma_brand_id': 0, 'buyma_brand_name': brand_en}

    def get_category_info(self, category_path: str, source_site: str = 'okmall') -> Dict:
        ...
        if category_path:
            self._register_unmapped_category(category_path, source_site=source_site)

        return {'buyma_category_id': 0, 'buyma_category_name': None}

    def _register_unmapped_category(self, category_path: str, source_site: str = 'okmall'):
        ...
        try:
            with self.engine.connect() as conn:
                existing = conn.execute(text(
                    "SELECT id FROM mall_categories WHERE full_path = :path AND mall_name = :mall_name"
                ), {'path': category_path, 'mall_name': source_site}).fetchone()

                if not existing:
                    conn.execute(text("""
                        INSERT INTO mall_categories
                        (mall_name, category_id, gender, depth1, depth2, depth3, full_path, buyma_category_id, is_active)
                        VALUES (:mall_name, :path, :gender, :d1, :d2, :d3, :path, NULL, 1)
                    """), {
                        'mall_name': source_site,
    ...
    def fetch_raw_data(self, limit: int = None, brand: str = None, raw_id: int = None, upsert: bool = False, source_site: str = None) -> List[Dict]:
        with self.engine.connect() as conn:
            if raw_id:
                ...
                # model_id별 best source 1건 선택
                # 우선순위: in_stock > 최저가 > 최신 updated_at
                query = """
                    SELECT r.id, r.source_site, r.mall_product_id, r.brand_name_en,
                           r.brand_name_kr, r.product_name, r.p_name_full, r.model_id,
                           r.category_path, r.original_price, r.raw_price, r.stock_status,
                        ...
                        FROM (
                            SELECT id, model_id,
                                   ROW_NUMBER() OVER (
                                       ...
                                       ORDER BY
                                           CASE WHEN stock_status = 'in_stock' THEN 0 ELSE 1 END,
                                           raw_price ASC,
                                           updated_at DESC
                                   ) as rn
                ...
                if brand:
                    # buyma_brand_name 기준으로 전체 source 브랜드 포함
                    query += """ AND UPPER(r.brand_name_en) IN (
                        SELECT UPPER(mall_brand_name_en) FROM mall_brands
                        WHERE UPPER(buyma_brand_name) = :brand
                          AND is_active = 1
                    )"""
    ...
    def convert_single_raw_to_ace(self, raw_data: Dict) -> Dict:
        ...
        category_info = self.get_category_info(raw_data.get('category_path', ''), source_site=raw_data.get('source_site', 'okmall'))
        ...
        # 자체 이미지 수집처: raw_json_data에서 이미지 URL 추출
        source_images = json_data.get('images', []) if self.has_own_images(raw_data.get('source_site', '')) else []

        return {'product': ace_product, 'options': ace_options, 'variants': ace_variants, 'source_images': source_images}

    def get_existing_ace_product(self, model_id: str) -> Optional[Dict]:
        """model_id(model_no)로 기존 ace_product 조회 (source 전환 대응)"""
        if not model_id:
            return None
        with self.engine.connect() as conn:
            result = conn.execute(text("""
                SELECT id, reference_number, buyma_product_id, is_published,
                       raw_data_id, source_site, is_buyma_locked
                FROM ace_products
                WHERE model_no = :model_id
            """), {'model_id': model_id})
            row = result.fetchone()
            if row:
                return {
                    ...
                    'raw_data_id': row[4],
                    'source_site': row[5],
                    'is_buyma_locked': row[6]
                }
            return None

    def update_ace_data(self, ace_data: Dict, existing_product: Dict) -> int:
        """기존 ace 데이터 업데이트 (가격, colorsize_comments, options, variants, source 전환)"""
        ace_product_id = existing_product['id']
        product = ace_data['product']

        # source 전환 감지
        source_changed = (existing_product.get('raw_data_id') != product.get('raw_data_id'))
        if source_changed:
            log(f"  → source 전환: {existing_product.get('source_site')} → {product.get('source_site')} "
                f"(raw_data_id: {existing_product.get('raw_data_id')} → {product.get('raw_data_id')})")

        with self.engine.connect() as conn:
            conn.execute(text("""
                UPDATE ace_products
                SET raw_data_id = :raw_data_id,
            ...
            """), {
                'raw_data_id': product.get('raw_data_id'),
    ...
    def save_ace_data(self, ace_data: Dict) -> int:
        with self.engine.connect() as conn:
            ...
            # 자체 이미지 수집처: raw_json_data의 이미지를 ace_product_images에 INSERT
            if self.has_own_images(product.get('source_site', '')):
                ...
                if source_images:
                    log(f"  → 자체 이미지 {len(source_images)}장 저장 (source: {product.get('source_site')})")
    ...
    def run_conversion(self, limit: int = None, brand: str = None, dry_run: bool = False, raw_id: int = None, upsert: bool = False, skip_translation: bool = False, source_site: str = None) -> Dict:
        ...
        for idx, raw_data in enumerate(raw_data_list):
            try:
                log(f"[{idx+1}/{len(raw_data_list)}] 변환 중: raw_id={raw_data['id']}, model_id={raw_data['model_id']}, source={raw_data['source_site']}...")

                # 기존 ace_product 확인 (model_id 기준 — source 전환 대응)
                existing_product = self.get_existing_ace_product(raw_data['model_id'])
...
def main():
    ...
    parser.add_argument('--source-site', type=str, default=None, help='특정 수집처만 처리 (예: okmall, kasina)')
    args = parser.parse_args()

    log("=" * 60)
    log("raw_to_ace_converter 시작")
    log(f"  옵션: brand={args.brand}, limit={args.limit}, dry_run={args.dry_run}, raw_id={args.raw_id}, upsert={args.upsert}, source_site={args.source_site}")
    log("=" * 60)

    try:
        converter = RawToAceConverter(DB_URL)
        result = converter.run_conversion(limit=args.limit, brand=args.brand, dry_run=args.dry_run, raw_id=args.raw_id, upsert=args.upsert, skip_translation=args.skip_translation, source_site=args.source_site)

# -*- coding: utf-8 -*-
"""
재고 및 가격 동기화 스크립트 (라프리마 버전)

바이마에 등록된 상품의 재고와 가격을 라프리마(laprima.co.kr)에서 재수집하여
ace 테이블을 업데이트하고 바이마 API로 상품을 수정합니다.

처리 흐름:
1. ace_products에서 바이마 등록 상품 조회 (is_published=1, buyma_product_id 있음)
2. 라프리마 상세 페이지 재방문 → 현재 가격/재고 수집
...
사용법:
    python stock_price_synchronizer_laprima_merge.py                    # 전체 실행
    python stock_price_synchronizer_laprima_merge.py --brand BURBERRY   # 특정 브랜드만
    python stock_price_synchronizer_laprima_merge.py --limit 100        # 최대 100개만
    python stock_price_synchronizer_laprima_merge.py --dry-run          # 테스트 (API 호출 안함)
    python stock_price_synchronizer_laprima_merge.py --force            # 변경 없어도 강제 API 호출
...
class StockPriceSynchronizer:
    ...
    # -------------------------------------------------
    # 2. 라프리마에서 가격/재고 수집 (HTML 스크래핑)
    # -------------------------------------------------
    def collect_from_laprima(self, product_url: str) -> Tuple[Dict, Optional[str]]:
        """
        라프리마 상세 페이지에서 가격/재고 수집

        - 세션 관리 + 타임아웃 감지 적용 (베이스 공통)
        - 30개마다 새 세션 + 메인 페이지 방문
        - 타임아웃 연속 5회 시 차단으로 판단
        - ★ 파싱은 laprima_collector.py 의 extract_detail_info/determine_stock_status 재사용:
          수집(COLLECT)→변환(CONVERT)에서 variants 에 저장한 source_option_code/size_value_original 과
          동일 코드로 옵션을 생성 → detect_stock_changes 매칭 100% 일치 (false out_of_stock 원천차단).
        """
        ...
        try:
            # ★ laprima_collector 의 파싱을 그대로 재사용 (COLLECT/CONVERT 와 동일 로직 → 옵션키 일치)
            import laprima_collector as lc

            detail = lc.extract_detail_info(html)

            result = {'original_price': 0, 'sale_price': 0, 'options': []}

            # 가격: build_raw_data 와 동일 우선순위
            #   raw_price(=매입가) = 회원혜택가 > 판매가 > 정상가
            #   original_price(=정가표시) = 정상가 > 판매가 > raw_price
            ...
            result['original_price'] = price_normal or price_sale or raw_price
            result['sale_price'] = raw_price  # 마진계산의 매입가로 사용됨

            # 옵션: extract_detail_info 의 {color, tag_size, option_code, status} →
            #   merge 매칭용 {color, size, option_code, status} 로 매핑 (tag_size→size)
            for o in detail.get('options', []):
                result['options'].append({
                    'color': o.get('color', '') or '',
                    'size': o.get('tag_size', 'FREE') or 'FREE',
                    'option_code': o.get('option_code', '') or '',
                    'status': o.get('status', 'in_stock'),
                })

            # 옵션 없는 단일 상품 → determine_stock_status 로 재고판정 (single_stock_number/SOLD OUT 폴백)
            #   CONVERT 의 단일 fallback variant(color=FREE,size=FREE,source_option_code=None)와 매칭되게 맞춘다.
            if not result['options']:
                stock_status = lc.determine_stock_status(detail, False, html)
                result['options'].append({
                    'color': '',
                    'size': 'FREE',
                    'option_code': '',
    ...
    def detect_stock_changes(self, db_variants: List[Dict], mall_options: List[Dict]) -> List[Dict]:
        ...
        # 단일 옵션 상품 처리: DB 1개, 라프리마 1개이면 이름 상관없이 직접 매칭
    ...
    def _mark_all_out_of_stock(self, ace_product_id: int) -> None:
        """laprima 품절/삭제/흠집 → 이 ace 의 옵션 전부 out_of_stock 표시.
    ...
    def _reconcile_published(self, products: List[Dict]) -> None:
        ...
        ★ 이번 synced 상품에 한정 (laprima published 전체 아님) → --limit/--id 테스트 안전.
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
    ...
    def process_single_product(self, product: Dict, idx: int, total: int, dry_run: bool, force: bool,
        ...
        try:
            # 1. 라프리마 HTML에서 가격/재고 수집
            mall_data, error = self.collect_from_laprima(product['source_product_url'])
            if error:
                add_log(f"  라프리마 수집 실패: {error}", "WARNING")
                ...
                # [MERGE] 바이마 직접 삭제 안 함 — laprima 옵션만 재고0 표시.
                #   laprima만 품절/삭제이어도 다른 몰 있으면 winner 이동, 없으면 reconcile 이 retire.
                add_log(f"  → 수집처 삭제/종료 → laprima 재고0 표시 (BUYMA 반영은 reconcile)")
                if not dry_run:
                    self._mark_all_out_of_stock(product['id'])
                    self.update_sync_time_only(product['id'])
                else:
                    add_log(f"  [DRY-RUN] laprima 재고0 표시 예정")
                with stats_lock:
                    stats['skipped'] += 1
...
def main():
    parser = argparse.ArgumentParser(description='바이마 재고/가격 동기화 (laprima)')

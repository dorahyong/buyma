...
# -*- coding: utf-8 -*-
"""
재고 및 가격 동기화 스크립트 (ABC마트 / 그랜드스테이지 버전)
...
사용법:
    python stock_price_synchronizer_abc.py --source abcmart                 # abcmart 전체 실행
    python stock_price_synchronizer_abc.py --source grandstage             # grandstage 전체 실행
    python stock_price_synchronizer_abc.py --source abcmart --brand NIKE   # 특정 브랜드만
    python stock_price_synchronizer_abc.py --source abcmart --limit 100    # 최대 100개만
    python stock_price_synchronizer_abc.py --source abcmart --dry-run      # 테스트 (API 호출 안함)
    python stock_price_synchronizer_abc.py --source abcmart --force        # 변경 없어도 강제 API 호출
...
def main():
    parser = argparse.ArgumentParser(description='바이마 재고/가격 동기화 (ABC마트 / 그랜드스테이지)')

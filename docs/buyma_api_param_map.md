...
| **게시 후 편집** | 가능. `id` 가 함께 나가도 **무시가 아니라 이 값으로 덮어써진다** (2026-08-05 실측) |
...
> ⚠️ **명세의 "무시된다"는 실제와 다르다.** 상품 API는 `id` 로 찾은 뒤 **같이 보낸 관리번호로 덮어쓴다**
> (2026-08-05 실측 → 아래 "상품번호·관리번호 두 칸의 경우의 수" 참고). 재고 API는 덮어쓰지 않는다.
...
# 상품번호·관리번호 두 칸의 경우의 수 (2026-08-05 재검증)

> 위 2026-08-04 표를 이어받아, **DB 두 칸이 비었거나 어긋났을 때 실제로 무슨 일이 벌어지는지**를
> 수정(상품 API)과 출품정지(재고 API) 각각에 대해 다시 쐈다. 결론부터 말하면
> **2026-08-04 에 "id 가 우선하고 관리번호는 무시된다"고 적은 것은 틀렸다.** 무시가 아니라 **덮어쓴다.**

## 어떻게 확인했나 (안전장치)

| | 내용 |
|---|---|
| 수정 테스트 대상 | `listing#42` / buyma `135126594` (A.P.C 청바지). 조회·찜·장바구니·판매 **전부 0** 인 상품 |
| 방법 | 프로덕션과 **똑같은 요청서**를 만들어 **가격만 1엔씩** 올려가며 정체성 칸만 바꿔 쐈다 |
| 재고 테스트 대상 | `listing#13056` / buyma `133662841` (이미 **출품정지중**) |
| 방법 | 프로덕션 출품정지 요청과 글자 그대로 같은 내용(전 옵션 `out_of_stock` + `order_quantity:0`). **이미 그 상태라 아무것도 안 바뀐다** |
| 안 한 것 | **존재하지 않는 `id`** 조합 (2026-08-04 사고 재발 방지). 틀린 값은 **존재하지 않는 관리번호**만 사용 |
| 뒷정리 | 두 상품 모두 원래 가격·관리번호로 복구 확인 완료 (`listing#42` 21,201엔 / 관리번호 원복) |
| 관찰 창구 | `buyma_listing_api_logs.last_api_call_at` + `buyma_listings.status/updated_at` (웹훅이 남기는 흔적) |

## ① 수정(상품 API) — 조합별 결과

| 보낸 조합 | HTTP | 웹훅 | BUYMA 반영 | 우리 DB |
|---|---|---|---|---|
| **id + 관리번호 (둘 다 정확)** ← 현행 프로덕션 | 201 | 성공 (2분 6초) | ✅ 가격 반영 | ✅ `status='success'` |
| **id 만** (관리번호 아예 안 보냄) | 201 | 성공 (4분 10초) | ✅ 가격 반영 | ✅ 갱신됨 — 웹훅이 **상품이 원래 갖고 있던 관리번호**를 실어 보내서 우리 행을 찾는다 |
| **관리번호만** (id 안 보냄, 값은 정확) | 201 | 성공 (2분 26초) | ✅ 가격 반영 (**새 상품이 생기지 않는다**) | ✅ 갱신됨 |
| **id 정확 + 관리번호 틀림** | 201 | **우리 쪽엔 안 옴** | ✅ 반영 — 그리고 **BUYMA 의 관리번호가 우리가 보낸 틀린 값으로 바뀐다** | ❌ 아무 기록 없음 |
| **둘 다 없음** | **422** 즉시 거부 | — | 없음 | 없음 |
| (참고) 정체성만 담은 최소 요청 | **422** | — | 없음 | 없음 — 수정에도 필수항목 12개를 **매번 전부** 보내야 한다 |

둘 다 없을 때의 422 응답: `{"errors":{"identity_requirements":["商品IDまたは商品管理番号を入力してください。"]}}`

### ★ 새로 밝혀진 것 — 관리번호는 "무시"가 아니라 "덮어쓰기"다

`id` 로 상품을 찾은 뒤, **같이 보낸 관리번호를 그 상품의 새 관리번호로 써 버린다.** 실측 순서:

```
15:57  관리번호만으로 수정 → 성공          (이 시점 관리번호 0ba0e00d… 는 유효)
16:01  id + 가짜 관리번호(ffffffff…)로 수정 → 201
16:12  재고 API에 원래 관리번호 0ba0e00d… → ❌ "商品管理番号は不正な値です"
       → BUYMA 는 더 이상 0ba0e00d… 를 모른다. 가짜 값으로 바뀐 것
16:13  id + 원래 관리번호로 다시 수정 → 성공
16:16  재고 API에 원래 관리번호 → ✅ 정상   → 원복 확인
```

**운영에 주는 뜻 (좋은 쪽)**: 우리 수정 요청은 **항상 `id` 와 관리번호를 함께** 보낸다
(`reconcile_buyma_push.build_edit_request`, `fast_price_updater.build_buyma_request`).
따라서 BUYMA 쪽 관리번호가 어쩌다 어긋나 있어도 **다음 수정 한 번이면 DB 값으로 자동 교정된다.**
게시중 82,067건에 어긋남이 사실상 없는 이유다.

**나쁜 쪽**: DB 관리번호가 틀린 값이면 그 틀린 값이 **BUYMA 로 밀려 들어간다.** 자동 교정이 반대로 작동한다.

## ② 재고 API(출품정지) — 조합별 결과

| 보낸 조합 | HTTP | 실제 적용 | 관리번호 변화 |
|---|---|---|---|
| **관리번호만 (정확)** ← 현행 프로덕션 | 201 | ✅ 적용 (7초 ~ 3분 40초) | 안 바뀜 |
| **id 만** | 201 | ✅ 적용 | 안 바뀜 |
| **id 정확 + 관리번호 틀림** | 201 | ✅ 적용 (id 로 찾는다) | **안 바뀜** ← 상품 API 와 다른 점 |
| **관리번호만 틀림 (id 없음)** | 201 | ❌ **8분 관찰, 아무 상품에도 닿지 않음** | — |
| **둘 다 없음** | **422** | 없음 | — |

- **재고 API 는 관리번호를 덮어쓰지 않는다.** 찾는 데만 쓴다. (상품 API 와 정반대)
- **2026-08-04 의 "개선 여지"는 유효하다.** `id` 만으로도, `id`+틀린 관리번호로도 정상 적용됐다.
  지금 출품정지는 관리번호만 보내므로, **`id` 를 함께 보내도록 고치면 관리번호가 어긋나도 안전해진다.**
- **틀린 관리번호 단독은 조용히 죽는다.** 201 을 받지만 아무 일도 없고, 웹훅도 우리 행을 못 찾으므로
  **DB 만 봐서는 성공했는지 실패했는지 영원히 알 수 없다.**

## ③ 웹훅이 우리 행을 찾는 방법 — 여기가 병목
`okmall_reference/server.py` 는 **오직 `reference_number` 로만** 우리 행을 찾는다
(`WHERE reference_number = %s`). 게다가 payload 에 관리번호가 없으면 **아무것도 안 하고 반환**한다.

| 상황 | 결과 |
|---|---|
| BUYMA 가 아는 관리번호 = DB `reference_number` | ✅ 정상 기록 |
| 우리가 `locked_reference_number` 를 보냈는데 그 값이 `reference_number` 와 다름 | ❌ **write-back 이 통째로 사라진다** (찾는 칸은 `reference_number` 뿐) |
| 관리번호가 BUYMA 와 어긋남 | ❌ 기록 없음 — 성공했는지도 모른다 |

즉 **두 관리번호 칸(`reference_number` / `locked_reference_number`)이 어긋나면, 보내는 건 잠긴 값인데 받는 건 현재 값으로 찾는다.** 어긋난 상품은 결과를 영원히 못 받는다.

## ④ DB 실측 (2026-08-05, 총 145,541건)

| 두 칸 상태 | 건수 | 게시중 | 뜻 |
|---|---|---|---|
| **둘 다 있음** | 91,641 | 82,067 | 정상. 게시중은 **전부** 두 칸을 갖고 있다 |
| **관리번호만 있음** (상품번호 없음) | 15,387 | 0 | 등록 요청은 보냈으나 확정 못 받음 (`fail` 11,176 · `api_error` 2,901 · `pending` 1,093 · 상태없음 217) |
| **둘 다 없음** | 38,513 | 0 | 아직 등록 안 한 것 |
| **상품번호만 있음** (관리번호 없음) | **0** | 0 | 없음 ✅ |

| 어긋남 항목 | 건수 | 비고 |
|---|---|---|
| `locked_reference_number` ≠ `reference_number` | **16** | 전부 미게시. 게시중엔 없음 ✅ |
| 게시중인데 `locked_reference_number` 가 빈 것 | 63 | 현재 값을 그대로 쓰므로 동작에는 지장 없음 |
| 같은 상품번호를 쓰는 목록이 둘 이상 | **0** | ✅ (단 DB에 유니크 제약은 없다 — 인덱스만 있음) |
| 게시중인데 마지막 웹훅 값과 DB 값이 다름 | 18 | 그룹 병합으로 관리번호를 다시 발급한 뒤 **옛 웹훅 기록이 남아 있는** 경우가 대부분 |
| 게시중인데 웹훅 기록이 아예 없음 | 48 | |
| **`pending` 인데 웹훅엔 상품번호가 있던 것** | **89** | 등록은 됐는데 우리 DB 가 상품번호를 못 받은 것 — **회수 가능** |

> **별개 주제(참고만)**: 게시중 82,067건 중 18,581건이 `status='fail'` 인데, 마지막 웹훅을 보면
> 원인은 두 칸의 정체성이 아니라 **내용**이다(상품명·코멘트에 한글 잔존 1,946+ · 변이 그리드 구멍 1,357 ·
> FREE 와 다른 사이즈 혼재 1,201 등). 이 문서의 주제 밖이라 여기서는 다루지 않는다.

## ⑤ 경우의 수 → 결과 한 장 요약

| DB 상태 | 수정(상품 API) | 출품정지(재고 API) |
|---|---|---|
| 둘 다 정확 | ✅ 정상 | ✅ 정상 |
| 둘 다 없음 | 코드가 **CREATE 로 보냄** (수정 대상 아님) | 코드가 **스킵** ("내릴 것 없음") |
| 상품번호 없음 + 관리번호 있음 | 코드가 **EDIT 안 함** (`buyma_product_id` 없으면 스킵) | 관리번호로 **시도한다** — BUYMA 가 모르는 값이면 조용히 실패 |
| 관리번호 없음 + 상품번호 있음 | API는 `id` 만으로 정상. **DB에 0건** | 현행 코드는 관리번호만 쓰므로 **스킵** (`id` 를 보내게 고치면 해결) |
| 관리번호가 BUYMA 와 다름 | ✅ 반영됨 (`id` 로 찾음) + **BUYMA 관리번호가 DB 값으로 교정됨**. 단 웹훅 write-back 은 실패할 수 있음 | ❌ **아무 데도 안 감 (조용한 실패)** |
| 상품번호가 BUYMA 와 다름 | ❌ 실패 웹훅 → **DB 상품번호가 지워지고 `status='fail'`** (2026-08-04 사고) | `id` 를 안 보내므로 영향 없음 |
| 두 관리번호 칸이 서로 다름 | 잠긴 값으로 보내고 현재 값으로 찾는다 → **결과를 못 받음** | 잠긴 값으로 보낸다 → BUYMA 가 그 값을 모르면 조용한 실패 |

## ⑥ 실측하지 않은 것 (근거만)

| 조합 | 왜 안 쐈나 | 구조상 결론 |
|---|---|---|
| **관리번호만 + BUYMA 가 모르는 값** (id 없음) | **새 상품이 실제로 만들어진다.** 중복 출품은 계정 위험 | 우리 **신규등록 요청과 글자 그대로 같은 모양**이다(차이는 `id` 유무뿐). → **중복 상품이 생긴다.** 지금은 수정 요청에 항상 `id` 를 넣어서 막혀 있다 |
| **관리번호가 다른 상품의 실제 값** | 남의 상품 두 개를 동시에 망가뜨림 | 관리번호는 유니크(`uk_reference_number`) → 거부되거나, 그 상품이 대상이 된다 |
| **존재하지 않는 `id`** | 2026-08-04 사고 재발 (DB 정체성 초기화) | 위 2026-08-04 표 참조 |

## ⑦ 부수 관찰

- **201 은 접수일 뿐**이고, 결과는 웹훅으로만 온다. 지연은 상품 API **2분 ~ 4분**, 재고 API **7초 ~ 3분 40초**.
- **재고 API 도 웹훅을 만든다.** 바뀐 게 없어도(이미 출품정지중인 상품에 다시 출품정지) 웹훅이 온다.
- 수정에도 필수항목을 **매번 전부** 보내야 한다. 일부만 보내는 부분 수정은 없다.

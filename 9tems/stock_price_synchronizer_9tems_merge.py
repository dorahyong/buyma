...
사용법:
    python stock_price_synchronizer_9tems_merge.py                    # 전체 실행
    python stock_price_synchronizer_9tems_merge.py --brand BURBERRY   # 특정 브랜드만
    python stock_price_synchronizer_9tems_merge.py --limit 100        # 최대 100개만
    python stock_price_synchronizer_9tems_merge.py --dry-run          # 테스트 (API 호출 안함)
    python stock_price_synchronizer_9tems_merge.py --force            # 변경 없어도 강제 API 호출
...
# 딜레이 설정
REQUEST_DELAY_MIN = 1.0   # 구템즈 요청 간 최소 딜레이
REQUEST_DELAY_MAX = 2.0   # 구템즈 요청 간 최대 딜레이
...
class StockPriceSynchronizer:
    ...
    # -------------------------------------------------
    # 2. 구템즈에서 가격/재고 수집 (HTML 스크래핑)
    # -------------------------------------------------
    def collect_from_9tems(self, product_url: str) -> Tuple[Dict, Optional[str]]:
        """
        구템즈(9tems) 상세 페이지에서 가격/재고 수집

        - #span_product_price_text(판매가) / #span_product_price_custom strike(정가)
        - select#product_option_id1 옵션 + [품절] 텍스트 + is_soldout_icon 로 재고 판정
          (9tems 수집기 parse_detail과 동일 → ace_product_variants 의 source_option_code/
           size_value_original 과 매칭됨)
        """
        ...
        try:
            ...
            # --- 가격 추출 (9tems: #span_product_price_text=판매가, #span_product_price_custom strike=정가) ---
            ...
            # --- 옵션/재고: select#product_option_id1 (9tems 수집기와 동일) ---
            #   select option value = ace_product_variants.source_option_code 와 동일하게 매칭됨.
            #   품절 판정: 전체품절(is_soldout_icon) or 옵션 텍스트 [품절] or disabled 속성.
            ...
            if option_select:
                for opt in option_select.select('option'):
                    ...
                    clean_size = normalize_size(re.sub(r'\s*\[품절\]\s*', '', opt_text).strip())
    ...
    def _mark_all_out_of_stock(self, ace_product_id: int) -> None:
        """9tems 품절/삭제/흠집 → 이 ace 의 옵션 전부 out_of_stock 표시.
    ...
    def _reconcile_published(self, products: List[Dict]) -> None:
        ...
        ★ 이번 synced 상품에 한정 (9tems published 전체 아님) → --limit/--id 테스트 안전.
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
    ...
    def process_single_product(self, product: Dict, idx: int, total: int, dry_run: bool, force: bool,
        ...
        try:
            # 1. 구템즈에서 가격/재고 수집
            mall_data, error = self.collect_from_9tems(product['source_product_url'])
            if error:
                add_log(f"  구템즈 수집 실패: {error}", "WARNING")
                ...
                # [MERGE] 바이마 직접 삭제 안 함 — 9tems 옵션만 재고0 표시.
                #   9tems만 품절/삭제이어도 다른 몰 있으면 winner 이동, 없으면 reconcile 이 retire.
                add_log(f"  → 수집처 삭제/종료 → 9tems 재고0 표시 (BUYMA 반영은 reconcile)")
                if not dry_run:
                    self._mark_all_out_of_stock(product['id'])
                    self.update_sync_time_only(product['id'])
                else:
                    add_log(f"  [DRY-RUN] 9tems 재고0 표시 예정")
                with stats_lock:
                    stats['skipped'] += 1
...
def main():
    parser = argparse.ArgumentParser(description='바이마 재고/가격 동기화 (9tems)')

# -*- coding: utf-8 -*-
"""
재고 및 가격 동기화 스크립트 (라벨루쏘 버전)

바이마에 등록된 상품의 재고와 가격을 라벨루쏘(labellusso.com)에서 재수집하여
ace 테이블을 업데이트하고 바이마 API로 상품을 수정합니다.

처리 흐름:
1. ace_products에서 바이마 등록 상품 조회 (is_published=1, buyma_product_id 있음)
2. 라벨루쏘 상세 페이지 재방문 → 현재 가격/재고 수집
...
# 딜레이 설정
REQUEST_DELAY_MIN = 1.0   # 라벨루쏘 요청 간 최소 딜레이
REQUEST_DELAY_MAX = 2.0   # 라벨루쏘 요청 간 최대 딜레이
...
class StockPriceSynchronizer:
    ...
    # -------------------------------------------------
    # 2. 라벨루쏘에서 가격/재고 수집 (HTML 스크래핑)
    # -------------------------------------------------
    def collect_from_labellusso(self, product_url: str) -> Tuple[Dict, Optional[str]]:
        """
        라벨루쏘 상세 페이지에서 가격/재고 수집

        - product_price / product_sale_price JS 변수에서 가격 추출
        - option_stock_data JS 변수 + #desctable에서 옵션/재고 추출
        """
        ...
        try:
            ...
            # --- 가격 추출 ---
            # product_price (consumer/original price)
            price_match = re.search(r"product_price\s*=\s*'(\d+)'", html)
            product_price = int(price_match.group(1)) if price_match else 0

            # product_sale_price (if > 0, this is the actual sale price)
            sale_match = re.search(r"product_sale_price\s*=\s*(\d+)", html)
            product_sale_price = int(sale_match.group(1)) if sale_match else 0

            # #span_product_price_custom for original/consumer price
            custom_elem = soup.select_one('#span_product_price_custom')
            custom_price = parse_price(custom_elem.get_text(strip=True)) if custom_elem else None

            # sale_price = product_sale_price if > 0, else product_price
            sale_price = product_sale_price if product_sale_price > 0 else product_price
            # original_price = custom price if exists, else product_price
            original_price = custom_price if custom_price else product_price
                ...
                for row in desctable.select('tr'):
                    ...
                    if len(tds) >= 2:
                        ...
                        if '사이즈' in header:
                            ...
                            desctable_sizes = [s.strip() for s in size_text.split(',') if s.strip()]

            # 사이즈 정규화 함수
            ONE_SIZE_VARIANTS = ['단일사이즈', '단일 사이즈', 'ONE SIZE', 'ONESIZE', '원사이즈']

            def normalize_size(size_val):
                if size_val.strip().upper() in [s.upper() for s in ONE_SIZE_VARIANTS]:
                    return 'FREE'
                return size_val.strip()

            # 3. 옵션 매칭
            if stock_data and desctable_sizes:
                # desctable_sizes가 있으면 각 사이즈를 stock_data와 매칭
                for size_raw in desctable_sizes:
                    size = normalize_size(size_raw)
                    status = 'out_of_stock'  # 기본값

                    # stock_data에서 해당 사이즈 찾기
            ...
            elif stock_data and not desctable_sizes:
                # desctable_sizes가 없으면 stock_data의 option_value를 직접 사용
                for opt_key, opt_info in stock_data.items():
                    if isinstance(opt_info, dict):
                        opt_value = opt_info.get('option_value', '')
                        is_selling = opt_info.get('is_selling', 'F')
                ...
                # Fallback: select[option_title] 태그에서 옵션 추출
                option_selects = soup.select('select[option_title]')
                for sel in option_selects:
                    for opt_tag in sel.select('option'):
                        opt_text = opt_tag.get_text(strip=True)
                        if not opt_text or opt_text.startswith('-') or opt_text.startswith('선택'):
                            continue

                        size = normalize_size(opt_text.split('(')[0].strip())
    ...
    def _mark_all_out_of_stock(self, ace_product_id: int) -> None:
        """labellusso 품절/삭제/흠집 → 이 ace 의 옵션 전부 out_of_stock 표시.
    ...
    def _reconcile_published(self, products: List[Dict]) -> None:
        ...
        ★ 이번 synced 상품에 한정 (labellusso published 전체 아님) → --limit/--id 테스트 안전.
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
    ...
    def process_single_product(self, product: Dict, idx: int, total: int, dry_run: bool, force: bool,
        ...
        try:
            # 1. 라벨루쏘에서 가격/재고 수집
            mall_data, error = self.collect_from_labellusso(product['source_product_url'])
            if error:
                add_log(f"  라벨루쏘 수집 실패: {error}", "WARNING")
                ...
                # [MERGE] 바이마 직접 삭제 안 함 — labellusso 옵션만 재고0 표시.
                #   labellusso만 품절/삭제이어도 다른 몰 있으면 winner 이동, 없으면 reconcile 이 retire.
                add_log(f"  → 수집처 삭제/종료 → labellusso 재고0 표시 (BUYMA 반영은 reconcile)")
                if not dry_run:
                    self._mark_all_out_of_stock(product['id'])
                    self.update_sync_time_only(product['id'])
                else:
                    add_log(f"  [DRY-RUN] labellusso 재고0 표시 예정")
                with stats_lock:
                    stats['skipped'] += 1
...
def main():
    parser = argparse.ArgumentParser(description='바이마 재고/가격 동기화 (labellusso)')

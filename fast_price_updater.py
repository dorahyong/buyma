...
최저가를 확보하거나, 마진 마이너스 시 출품정지중(품절) 처리하는 스크립트.
...
4. 내가 최저가 아님 → DB의 purchase_price_krw 기준 마진 계산
   - 마진 + → 가격 인하 (바이마 API 수정)
   - 마진 - → 바이마 재고 API 출품정지중(전 옵션 품절, 삭제 아님 → buyma_product_id 유지)
...
# 매입처 설명 글자수 상한. 초과하면 BUYMA 가 422 로 거부한다:
...
def shop_url_description(price_krw, stocks) -> str:
    ...
    ★ okmall/reconcile_buyma_push.shop_url_description 과 같은 규칙 —
      한쪽만 고치면 서로 덮어쓴다.
    """
...
# 적응형 주기 스케줄러 설정
MAX_INTERVAL_MIN = 360       # 베이스라인(안정 상품 수렴 주기) = 6시간
MIN_INTERVAL_MIN = 2         # 접전 상품 최소 주기 = 2분
RATE_PER_SEC = 4.0           # 바이마 조회 호출 상한(전체 합산) = 초당 4회
BATCH_SIZE = 400             # 한 배치 크기(가장 밀린 주기부터). 재조회 주기 ≈ BATCH_SIZE/RATE 초
IDLE_SLEEP_SEC = 30          # due 0건일 때 대기


class RateLimiter:
    """전체 호출 속도를 per_sec 로 제한하는 스레드 안전 리미터(토큰 간격 방식)."""
...
def _next_interval(cur, kept: bool) -> int:
    """다음 조회 주기 계산(순수 함수). 유지=×2(상한 MAX), 빼앗김=÷2(하한 MIN)."""
...
def log(msg: str, level: str = "INFO") -> None:
    ...
    # 파일 로그: 1일 1파일(append, 시간 없는 날짜파일). [스킵] 라인은 파일에 안 쌓음.
    if "[스킵]" in msg:
        return
    try:
...
class FastPriceUpdater:
    ...
    def get_products(self, limit: int = None, brand: str = None,
                     source: str = None, product_id: int = None,
        ...
        try:
            with conn.cursor() as cursor:
                ...
                # 적응형 스케줄: 지금 체크할 때가 된 것만 (next_price_check_at NULL=미배정→즉시 대상)
                if respect_schedule:
                    sql += " AND (ap.next_price_check_at IS NULL OR ap.next_price_check_at <= NOW())"
                ...
                # 주기 짧은 순(=Rate Monotonic, 접전 상품 우선) → 같은 주기면 가장 밀린 순
                sql += " ORDER BY ap.price_check_interval_min ASC, ap.next_price_check_at ASC, ap.id ASC"
    ...
    def get_product_data_for_api(self, ace_product_id: int) -> Dict:
        ...
        try:
            with conn.cursor() as cursor:
                ...
                # 매입처 전부(병합 상품은 여럿). 이걸 안 보내면 reconcile 이 넣은
                #   15칸이 여기서 1칸으로 덮어써진다. 같은 커서 재사용(새 연결 안 엶).
                cursor.execute("""
                    SELECT so.listing_id, bl.winner_offering_id
                    FROM source_offerings so
                    JOIN buyma_listings bl ON bl.id = so.listing_id
                    WHERE so.ace_product_id = %s AND so.is_active = 1
                    LIMIT 1
                """, (ace_product_id,))
                lrow = cursor.fetchone()
                shop_urls = (self.build_shop_urls(cursor, lrow['listing_id'], lrow['winner_offering_id'])
                             if lrow else [])

                return {'product': product, 'images': images, 'options': options,
                        'variants': variants, 'shop_urls': shop_urls}
        finally:
            conn.close()

    def build_shop_urls(self, cursor, listing_id, winner_offering_id) -> list:
        """이 목록의 소싱처 전부 → BUYMA shop_urls 배열.
        winner 맨 앞, 나머지는 매입가 싼 순. 상한 15칸.
        ★ reconcile_buyma_push._shop_urls 와 같은 규칙 — 한쪽만 고치면 서로 덮어쓴다."""
        cursor.execute("""
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
        ...
        # images
        images_arr = [{"path": row['cloudflare_image_url'], "position": idx} for idx, row in enumerate(images, start=1)]
        ...
        # 매입처 전부(병합 상품은 여럿). 목록이 없을 때만 종전대로 winner 1칸.
        if data.get('shop_urls'):
            request_data['shop_urls'] = data['shop_urls'][:MAX_SHOP_URLS]
    ...
    def call_buyma_variants_soldout(self, reference_number: str, variant_rows: list) -> Dict:
        """재고 API(variants.json)로 전 옵션 품절(out_of_stock) → '출품정지중'(삭제 아님).
        buyma_product_id 유지 → 마진 회복 시 같은 상품으로 출품중 복구.
        order_quantity=0 필수(전 변이 out_of_stock 시 기존 order_quantity 충돌 방지).
        실제 is_published=0 은 buyer_suspended webhook 도 반영(server.py). (register.call_buyma_variants_soldout 와 동일)"""
    ...
    def update_after_soldout(self, ace_product_id: int) -> None:
        """출품정지중(품절) 후 DB 반영. 삭제 아님 → buyma_product_id 유지.
        is_published 1→0 전이는 게시일수 트리거가 reason='soldout' 으로 기록."""
        conn = self.get_connection()
        try:
            with conn.cursor() as cursor:
                cursor.execute("""
    ...
    def _reschedule(self, ace_product_id: int, kept: bool, cur_interval_min) -> None:
        """체크 결과로 다음 조회 주기 갱신. 유지=×2(상한 360=6h), 빼앗김=÷2(하한 2).
        next_price_check_at 에는 ±15% jitter(초 단위)를 더해 동기화 덩어리(클럼프)를 분산.
        interval 컬럼은 깨끗한 값(240/120/…) 유지 — ×2/÷2 계산 기준이라 흔들지 않음."""
        new_interval = _next_interval(cur_interval_min, kept)
        # 예약 시각에만 ±15% 흔들기 (초 단위). 하한 10분(=600초)이라 음수/과거 위험 없음.
        base_sec = new_interval * 60
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute("""
                    UPDATE ace_products
                    SET price_check_interval_min = %s,
                        next_price_check_at = NOW() + INTERVAL %s SECOND
                    WHERE id = %s
                """, (new_interval, jittered_sec, ace_product_id))
                conn.commit()
        finally:
            conn.close()

    def _reschedule_error(self, ace_product_id: int, minutes: int = 30) -> None:
        """조회 실패 시: 주기는 그대로, 다음 시각만 30분 뒤로 (재시도)."""
        conn = self.get_connection()
        try:
            with conn.cursor() as cursor:
                cursor.execute("""
                    UPDATE ace_products
                    SET next_price_check_at = NOW() + INTERVAL %s MINUTE
                    WHERE id = %s
                """, (minutes, ace_product_id))
                conn.commit()
        finally:
            conn.close()
    ...
    def process_single_product(self, product: Dict, idx: int, total: int,
        ...
        cur_interval = product.get('price_check_interval_min')
        ...
        # 바이마 최저가 조회 (전체 호출속도 RATE_PER_SEC 로 제한)
        ...
        if error:
            if "경쟁자 없음" in error or "검색 결과 없음" in error:
                ...
                if not dry_run:
                    self._reschedule(ace_id, kept=True, cur_interval_min=cur_interval)
                log(f"{prefix} [스킵] 경쟁자 없음 (최저가) | 주기 {cur_interval or MAX_INTERVAL_MIN}→{_next_interval(cur_interval, True)}분")
                with stats_lock:
        ...
        if current_price <= competitor_price:
            ...
            if price_range_min <= current_price <= price_range_max:
                ...
                if not dry_run:
                    self._reschedule(ace_id, kept=True, cur_interval_min=cur_interval)
                log(f"{prefix} [스킵] 이미 최저가 (내 ¥{current_price:,} | 경쟁자 ¥{competitor_price:,} | 범위 내) | 주기 {cur_interval or MAX_INTERVAL_MIN}→{_next_interval(cur_interval, True)}분")
                with stats_lock:
                    stats['already_lowest'] += 1
                return
            ...
            log(f"{prefix} [조정] 최저가지만 gap 큼: ¥{current_price:,}→¥{new_price_jpy:,} (경쟁자 ¥{competitor_price:,}) | 주기 {cur_interval or MAX_INTERVAL_MIN}→{_next_interval(cur_interval, True)}분")
            ...
            self._reschedule(ace_id, kept=True, cur_interval_min=cur_interval)  # 내가 최저(gap 조정) → 유지
            ...
            if result.get('success'):
                ...
                log(f"  → API 실패: status={result.get('status_code', 'N/A')} body={result.get('error', 'Unknown')!r}", "ERROR")
                with stats_lock:
                    stats['api_failed'] += 1
        ...
        if margin_info['is_profitable']:
            ...
            log(f"{prefix} [인하] ¥{current_price:,}→¥{new_price_jpy:,} (경쟁자 ¥{competitor_price:,}) | "
                ...
                f"주기 {cur_interval or MAX_INTERVAL_MIN}→{_next_interval(cur_interval, False)}분")
            ...
            self.update_price_in_db(ace_id, new_price_jpy, competitor_price,
                                     margin_info['margin_rate'], margin_info['margin_krw'])
            self._reschedule(ace_id, kept=False, cur_interval_min=cur_interval)  # 빼앗김 → 자주
            ...
            # 마진 - → 출품정지중(재고 API). 삭제 아님 → buyma_product_id 유지, 마진 회복 시 같은 상품으로 복구.
            log(f"{prefix} [출품정지중] 마진 마이너스 ₩{margin_info['margin_krw']:,.0f} (경쟁자 ¥{competitor_price:,})", "WARNING")
            ...
            # 재고 API: 전 옵션 out_of_stock + order_quantity:0 (변이 = 상품의 모든 색/사이즈)
            api_data = self.get_product_data_for_api(ace_id)
            reference_number = (api_data['product'].get('locked_reference_number')
                                or api_data['product'].get('reference_number'))
            variant_rows = api_data['variants']
            if not reference_number or not variant_rows:
                log(f"  → 출품정지 스킵 (ref 또는 옵션 없음)", "WARNING")
                with stats_lock:
                    stats['skipped'] += 1
                return

            result = self.call_buyma_variants_soldout(reference_number, variant_rows)
            self.update_api_log(ace_id,
                                {"product": {"reference_number": reference_number, "control": "variants_soldout"}},
                                result)

            if result.get('success'):
                self.update_after_soldout(ace_id)
                with stats_lock:
                    stats['soldout'] += 1
                    stats['api_called'] += 1
            else:
                log(f"  → 출품정지 API 실패: {result.get('error', 'Unknown')}", "ERROR")
                with stats_lock:
                    stats['api_failed'] += 1
    ...
    def run(self, limit: int = None, brand: str = None, source: str = None,
        ...
        log(f"  병렬 {DEFAULT_WORKERS} · 레이트 {RATE_PER_SEC}/초 · 배치 {BATCH_SIZE} · "
            f"베이스라인 {MAX_INTERVAL_MIN}분({MAX_INTERVAL_MIN // 60}h) · 최소 {MIN_INTERVAL_MIN}분")
        log("=" * 60)

        # 수동 타깃(--id/--brand/--source)은 스케줄 무시, 기본 루프는 스케줄 존중
        respect_schedule = (product_id is None and brand is None and source is None)

        if count_only:
            products = self.get_products(limit=limit, brand=brand, source=source,
                                         product_id=product_id, respect_schedule=respect_schedule)
            site_counts = {}
            for p in products:
        ...
        if not respect_schedule:
            products = self.get_products(limit=limit, brand=brand, source=source,
                                         product_id=product_id, respect_schedule=False)
            log(f"대상 상품: {len(products)}건")
            if not products:
                log("대상 상품이 없습니다.")
                return {'total': 0}
            stats = self._process_products(products, dry_run)
            log("=" * 60)
            log(f"완료: 인하 {stats['price_lowered']} · 유지 {stats['already_lowest']} · "
                f"출품정지 {stats['soldout']} · 오류 {stats['error']} (총 {stats['total']})")
            log("=" * 60)
            return stats

        # 기본 모드: 주기 짧은 순(Rate Monotonic) 연속 배치 스케줄러 (무한 루프)
        log("스케줄러 모드: 주기 짧은 순 우선, 배치마다 재조회 (Ctrl+C 로 중단)")
        cum = {'processed': 0, 'price_lowered': 0, 'already_lowest': 0, 'soldout': 0, 'error': 0}
        batch_no = 0
        while True:
            products = self.get_products(limit=BATCH_SIZE, respect_schedule=True)
            if not products:
            ...
            iv_min = products[0].get('price_check_interval_min')
            iv_max = products[-1].get('price_check_interval_min')
    ...
    def _process_products(self, products, dry_run: bool) -> Dict:
        """상품 배치를 워커 풀로 처리하고 stats 반환. (호출 속도는 RateLimiter 가 제한)"""
        stats = {
            'total': len(products), 'already_lowest': 0, 'price_lowered': 0,
        ...
        with ThreadPoolExecutor(max_workers=DEFAULT_WORKERS) as executor:
            futures = [
                executor.submit(self.process_single_product,
                                product, idx + 1, len(products), dry_run, stats, stats_lock)

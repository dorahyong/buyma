...
def check_scratch_product(product_url: str) -> bool:
    """
    오케이몰 재방문하여 흠집 상품인지 확인
    ...
    try:
        ...
        if 'item_scratch' in response.text:
            return True
        return False
    except Exception as e:
        log(f"흠집 체크 실패: {e}", "WARNING")
        return False

def delete_scratch_product(conn, product: Dict) -> bool:
    """
    흠집 상품 삭제 (바이마 + DB)
    """
    ...
    log(f"  → 흠집 상품 발견! 삭제 진행... (ace_product_id={product_id})")
    
    try:
        cursor = conn.cursor()
        
        # 1. 바이마에 등록된 상품이면 바이마에서 삭제
        if buyma_product_id:
            log(f"  → 바이마 상품 삭제 필요: {buyma_product_id}")
            # TODO: 바이마 삭제 API 호출 (필요시 구현)
        
        # 2. ace 관련 테이블 삭제
        cursor.execute("DELETE FROM ace_product_variants WHERE ace_product_id = %s", (product_id,))
        cursor.execute("DELETE FROM ace_product_options WHERE ace_product_id = %s", (product_id,))
        cursor.execute("DELETE FROM ace_product_images WHERE ace_product_id = %s", (product_id,))
        cursor.execute("DELETE FROM ace_products WHERE id = %s", (product_id,))
        
        # 3. raw_scraped_data 삭제
        if raw_data_id:
...
def get_products_to_register(conn, limit: int = 10, brand: str = None, product_id: int = None) -> List[Dict]:
    """
    등록/수정 대상 상품 조회
    - locked_* 컬럼: 바이마 등록 후 불변 필드 백업값
    - is_buyma_locked: 불변 필드 잠금 여부
    """
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
    ...
    # 불변 필드 결정: 이미 등록된 상품(수정 요청)이면 locked_* 값 사용
    is_update_request = product.get('is_published') == 1 and product.get('buyma_product_id')
    
    if is_update_request and product.get('is_buyma_locked') == 1:
        # 수정 요청: 불변 필드는 locked_* 값 사용 (바이마 API 오류 방지)
    ...
    request_data = {
        # 필수 필드 (불변 필드는 위에서 결정된 api_* 값 사용)
        "control": "publish",
    ...
    # brand_id=0인 경우 (바이마 미등록 브랜드) brand_name 필드 추가
    # 바이마 API 규칙: brand_id=0일 때 brand_name으로 커스텀 브랜드명 지정 가능
...
def main():
    ...
    try:
        ...
        for i, product in enumerate(products, 1):
            ...
            try:
                # 흠집 상품 체크
                source_url = product.get('source_product_url')
                if source_url and check_scratch_product(source_url):
                    delete_scratch_product(conn, product)
                    scratch_count += 1
                    continue
        ...
        log("\n" + "=" * 60)
        log("처리 완료")
        log(f"성공: {success_count}, 스킵: {skip_count}, 실패: {fail_count}, 흠집삭제: {scratch_count}")
        log("=" * 60)

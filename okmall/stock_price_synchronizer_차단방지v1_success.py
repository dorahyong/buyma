...
# =====================================================
# ★★★ 완전히 새로운 브라우저처럼 오케이몰 접속 ★★★
# =====================================================

def fetch_okmall_like_browser(product_url: str) -> Tuple[Optional[str], Optional[str]]:
    """
    실제 브라우저처럼 오케이몰에 접속합니다.
    
    1. 완전히 새로운 세션 생성 (쿠키 없음)
    2. 랜덤 브라우저 프로필 선택
    3. 메인 페이지 먼저 방문 (쿠키 획득)
    4. 짧은 대기 후 상품 페이지 방문
    ...
    try:
        ...
        # ★ 2. 랜덤 브라우저 프로필 선택 (하나의 세션에서 일관된 프로필 사용)
        profile = random.choice(BROWSER_PROFILES).copy()
        
        # ★ 3. 메인 페이지 먼저 방문 (쿠키 획득 + 정상 접속 흔적)
        main_headers = profile.copy()
        ...
        main_response = session.get('https://www.okmall.com/', timeout=15)
        
        if main_response.status_code != 200:
            return None, f"메인 페이지 접속 실패: {main_response.status_code}"
        
        # ★ 4. 짧은 대기 (사람처럼)
        time.sleep(random.uniform(0.5, 1.5))
        
        # ★ 5. 상품 페이지 방문 (쿠키 유지, Referer 변경)
        product_headers = profile.copy()
        product_headers['Referer'] = 'https://www.okmall.com/'  # 메인에서 온 것처럼
        product_headers['Sec-Fetch-Site'] = 'same-origin'  # 같은 사이트 내 이동
        
        # 헤더 업데이트 (쿠키는 세션이 자동 유지)
...
class StockPriceSynchronizer:
    ...
    # -------------------------------------------------
    # 2. 오케이몰에서 가격/재고 수집 (★ 봇 감지 방지 적용)
    # -------------------------------------------------
    def collect_from_okmall(self, product_url: str) -> Tuple[Dict, Optional[str]]:
        """★ 브라우저처럼 접속하여 수집"""
        
        html, error = fetch_okmall_like_browser(product_url)
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
    ...
    def process_single_product(self, product: Dict, idx: int, total: int, dry_run: bool, force: bool,
        ...
        add_log(f"\n[{idx}/{total}] {product['brand_name']} - {product['name'][:30]}...")

        try:
            # 1. 오케이몰 가격/재고 수집 (★ 봇 감지 방지 적용)
    ...
    def run(self, limit: int = None, brand: str = None, product_id: int = None, dry_run: bool = False, force: bool = False) -> Dict:
        log("=" * 60)
        log("재고/가격 동기화 시작 (강화된 봇 감지 방지 버전)")

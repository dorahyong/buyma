...
def get_products_to_register(conn, limit: int = None, brand: str = None, product_id: int = None) -> List[Dict]:
    """
    ★★★ 신규 등록 대상 상품 조회 ★★★
...
def build_options_array(option_rows: List[Dict], valid_sizes: set = None, valid_colors: set = None) -> List[Dict]:
    """
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
    ...
    # ★ options 필터링: variants에 있는 size/color만 포함
    filtered_options = build_options_array(options, valid_sizes, valid_colors)

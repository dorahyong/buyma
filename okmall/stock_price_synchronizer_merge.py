...
사용법:
    python stock_price_synchronizer_merge.py                    # 전체 실행
    python stock_price_synchronizer_merge.py --brand BURBERRY   # 특정 브랜드만
    python stock_price_synchronizer_merge.py --limit 100        # 최대 100개만
    python stock_price_synchronizer_merge.py --dry-run          # 테스트 (API 호출 안함)
    python stock_price_synchronizer_merge.py --force            # 변경 없어도 강제 API 호출
...
# [MERGE] reconcile 엔진을 여기서 import.
...
class StockPriceSynchronizer:
    ...
    def collect_from_okmall(self, product_url: str) -> Tuple[Dict, Optional[str]]:
        ...
        with self.session_lock:
            ...
            # 상품 페이지 접속 — 일시적 실패는 재시도한다.
            #   렉스몬드(구 오케이몰)는 삭제된 상품도 HTTP 200 으로 응답한다(2026-07-29 실측).
            #   즉 여기서 잡히는 error 는 사실상 전부 일시적 장애다. 재시도 없이 실패로 넘기면
            #   호출부가 '수집처 삭제' 로 보고 그 상품 전 옵션을 재고0 으로 찍어버린다.
            ...
            for attempt in range(FETCH_MAX_RETRIES + 1):
                ...
                if not error:
                    break
                if error == "접근 차단됨 (403)":
                    break          # 차단은 재시도 대상이 아니다 (호출부가 즉시 중단)
                if attempt < FETCH_MAX_RETRIES:
                    time.sleep(3 * (attempt + 1))

            # ★ 타임아웃 연속 감지 (상품 1건당 1회. 재시도 횟수만큼 세지 않는다)
        ...
        if error:
            # 403·타임아웃차단은 위에서 이미 걸러져 나갔다. 여기 남는 건 재시도까지 다 쓴
            # 일시적 장애뿐 → 재고를 건드리지 말고 이번 회차만 건너뛰라고 호출부에 알린다.
            if error == "접근 차단됨 (403)":
                return {}, error
            return {}, f"일시적 오류 (스킵): {error}"

        if not html:
            return {}, "일시적 오류 (스킵): 빈 응답"

        try:
            soup = BeautifulSoup(html, 'html.parser')
            
            # 흠집 상품 체크 (상세 페이지에서는 scratch-banner 클래스로 표시됨)
            if 'scratch-banner' in html:
                return {}, "흠집 상품"
            ...
            # 페이지는 받았는데 못 읽은 경우. 삭제된 게 아니므로 재고를 건드리면 안 된다.
    ...
    def _mark_all_out_of_stock(self, ace_product_id: int) -> None:
        """okmall 품절/삭제/흠집 → 이 ace 의 옵션 전부 out_of_stock 표시.
    ...
    def _reconcile_published(self, products: List[Dict]) -> None:
        ...
        ★ 이번 synced 상품에 한정 (okmall published 전체 아님) → --limit/--id 테스트 안전.
    ...
    def process_single_product(self, product: Dict, idx: int, total: int, dry_run: bool, force: bool,
        ...
        try:
            ...
            if error:
                ...
                # ★ 일시적 오류 → 재고를 건드리지 않고 이번 회차만 건너뛴다.
                #   여기서 재고0 을 찍으면 렉스몬드가 잠깐 느린 것만으로 멀쩡한 상품이
                #   BUYMA 에서 출품정지된다(okmall 단독 상품이면 그대로 내려감).
                if "일시적 오류" in error:
                    add_log(f"  → 일시적 오류, 이번 회차 스킵 (재고 그대로 둠)")
                    with stats_lock:
                        stats['skipped'] += 1
                    log_batch(logs)
                    random_delay()
                ...
                # [MERGE] 바이마 직접 삭제 안 함 — okmall 옵션만 재고0 표시.
                #   okmall만 품절/삭제/흠집이어도 다른 몰 있으면 winner 이동, 없으면 reconcile 이 retire.
                if error == "흠집 상품":
                    add_log(f"  → 흠집 상품 → okmall 재고0 표시 (BUYMA 반영은 reconcile)")
                else:
                    add_log(f"  → 수집처 삭제/종료 → okmall 재고0 표시 (BUYMA 반영은 reconcile)")
                if not dry_run:
                    self._mark_all_out_of_stock(product['id'])
                    self.update_sync_time_only(product['id'])
                else:
                    add_log(f"  [DRY-RUN] okmall 재고0 표시 예정")
                with stats_lock:
                    stats['skipped'] += 1

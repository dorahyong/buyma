...
작성일: 2026-01-23
수정일: 2026-01-28 - DeepL API로 변경 (월 50만 글자 무료)
"""
...
DEEPL_API_URL = "https://api-free.deepl.com/v2/translate"  # 무료 버전
...
def translate_with_deepl(text: str, max_retries: int = 3) -> str:
    """
    DeepL API를 사용하여 한국어를 일본어로 번역
    ...
    # 캐시 확인
    cache_key = text[:500]  # 캐시 키는 앞 500자로 제한
    if cache_key in _translation_cache:
        return _translation_cache[cache_key]

    if not DEEPL_API_KEY:
        log("DeepL API 키가 설정되지 않았습니다.", "WARNING")
        return text
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                log(f"DeepL API 한도 초과, {wait_time}초 대기 후 재시도... ({attempt + 1}/{max_retries})", "WARNING")
                time.sleep(wait_time)
            elif response.status_code == 456:
                log("DeepL 무료 할당량 소진. 다음 달까지 대기 또는 유료 전환 필요.", "ERROR")
                return text
            else:
                log(f"DeepL API 오류 (HTTP {response.status_code}): {response.text[:200]}", "ERROR")
                return text

        except requests.exceptions.Timeout:
            log(f"DeepL API 타임아웃, 재시도... ({attempt + 1}/{max_retries})", "WARNING")
            time.sleep(2)
        except Exception as e:
            log(f"DeepL 번역 실패: {e}", "ERROR")
            return text

    log(f"DeepL 번역 최대 재시도 초과, 원본 반환", "WARNING")
    return text
...
def convert_to_japanese(text: str) -> str:
    ...
    if contains_korean(result):
        result = translate_with_deepl(result)

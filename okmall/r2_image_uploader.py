...
class R2ImageUploader:
    ...
    def fetch_pending_images(self, limit: int = None, retry_failed: bool = False) -> List[ImageRecord]:
        """
        업로드 대기 중인 이미지 조회
        ...
        with self.engine.connect() as conn:
            if retry_failed:
                ...
                query = """
                    SELECT id, ace_product_id, position, source_image_url, cloudflare_image_url, is_uploaded
                    FROM ace_product_images
                    WHERE source_image_url IS NOT NULL
                      AND source_image_url != :not_found
                ...
                query = """
                    ...
                    WHERE source_image_url IS NOT NULL
                      AND source_image_url != :not_found
                      AND source_image_url != ''
                      AND (cloudflare_image_url IS NULL OR cloudflare_image_url = '')
                      AND is_uploaded = 0
                    ORDER BY ace_product_id, position
                """
            ...
            result = conn.execute(text(query), {'not_found': NOT_FOUND_VALUE})
    ...
    def run(self, limit: int = None, retry_failed: bool = False) -> Dict:
        """
        전체 실행
        ...
        # 대기 중인 이미지 조회
        images = self.fetch_pending_images(limit=limit, retry_failed=retry_failed)

# -*- coding: utf-8 -*-
"""
썸네일 생성기 — 상품 대표 이미지에 뱃지(送料・関税込 / 追跡付)를 얹어
...
┌─────────────────────────────────────────────────────────────────────┐
│ ★ 혜지님 확인 필요(자리 비우셔서 추천값으로 세팅해 둠 — 오시면 조정) │
│  - 대상: 대표 이미지(position=1)만                                     │
│  - 뱃지 PNG: '그림자 없는 투명 PNG' 가정 → 그림자는 코드에서 생성      │
│  - 배치: 좌측 하단, 아래→위로 세로 스택(ARRANGEMENT='vertical')        │
│  - 등록 연동(대표이미지 교체)은 이 파일에서 안 함(다음 단계)           │
└─────────────────────────────────────────────────────────────────────┘
"""
import argparse
import os

...
사용법:
    python stock_price_synchronizer_kasina_merge.py                    # 전체 실행
    python stock_price_synchronizer_kasina_merge.py --brand BURBERRY   # 특정 브랜드만
    python stock_price_synchronizer_kasina_merge.py --limit 100        # 최대 100개만
    python stock_price_synchronizer_kasina_merge.py --dry-run          # 테스트 (API 호출 안함)
    python stock_price_synchronizer_kasina_merge.py --force            # 변경 없어도 강제 API 호출
...
# 딜레이 설정
REQUEST_DELAY_MIN = 0.5  # 카시나 API 요청 간 최소 딜레이
REQUEST_DELAY_MAX = 1.0  # 카시나 API 요청 간 최대 딜레이
...
def kasina_api_get(base_url: str, endpoint: str, params: dict = None) -> Tuple[Optional[dict], Optional[str]]:
    """
    카시나 Shopby API GET 요청

    Returns:
        (data, error): 성공 시 (data, None), 404 시 (None, "NOT_FOUND"), 기타 오류 시 (None, 에러메시지)
    """
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            log(f"  카시나 API 오류 {resp.status_code}: {endpoint}", "WARNING")
            return None, f"API 오류 ({resp.status_code})"
    except requests.exceptions.Timeout:
        log(f"  카시나 API 타임아웃: {endpoint}", "WARNING")
        return None, "API 타임아웃"
    except Exception as e:
        log(f"  카시나 API 요청 실패: {endpoint} - {e}", "ERROR")
...
class StockPriceSynchronizer:
    ...
    # -------------------------------------------------
    # 2. 카시나에서 가격/재고 수집 (Shopby API)
    # -------------------------------------------------
    def collect_from_kasina(self, product_url: str) -> Tuple[Dict, Optional[str]]:
        """
        카시나 Shopby API로 가격/재고 수집

        - 상세 API: 정가(salePrice), 할인가(immediateDiscountAmt)
        - 옵션 API: 사이즈별 재고 상태(flatOptions → saleType)
        """
        # product_url에서 product_no 추출
        # 형식: https://www.kasina.co.kr/product-detail/{product_no}
        match = re.search(r'/product-detail/(\d+)', product_url)
        if not match:
            return {}, f"URL에서 product_no 추출 실패: {product_url}"

        product_no = match.group(1)

        # 상세 API 호출
        detail, api_err = kasina_api_get(KASINA_LIST_API_BASE, f'/products/{product_no}', {'useCache': 'false'})
        if not detail:
            if api_err == "NOT_FOUND":
                return {}, "상품 삭제됨 (404)"
            else:
                return {}, f"일시적 오류 (스킵): {api_err}"

        time.sleep(random.uniform(0.3, 0.5))

        # 옵션 API 호출
        options_data, _ = kasina_api_get(KASINA_OPTIONS_API_BASE, f'/products/{product_no}/options', {'useCache': 'false'})
        ...
        try:
            ...
            sale_price = int(price_info.get('salePrice', 0) or 0)
            immediate_discount = int(price_info.get('immediateDiscountAmt', 0) or 0)

            result['original_price'] = sale_price
            result['sale_price'] = sale_price - immediate_discount

            # 판매 상태 확인 (status 객체에 위치)
            status_info = detail.get('status', {})
            sale_status = status_info.get('saleStatusType', '')
            if sale_status not in ('ONSALE', 'READY'):
                return {}, "판매 종료 상품"
    ...
    def _mark_all_out_of_stock(self, ace_product_id: int) -> None:
        """kasina 품절/삭제/흠집 → 이 ace 의 옵션 전부 out_of_stock 표시.
    ...
    def _reconcile_published(self, products: List[Dict]) -> None:
        ...
        ★ 이번 synced 상품에 한정 (kasina published 전체 아님) → --limit/--id 테스트 안전.
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
    ...
    def process_single_product(self, product: Dict, idx: int, total: int, dry_run: bool, force: bool,
        ...
        try:
            # 1. 카시나 API로 가격/재고 수집
            mall_data, error = self.collect_from_kasina(product['source_product_url'])
            if error:
                add_log(f"  카시나 수집 실패: {error}", "WARNING")
                ...
                # [MERGE] 바이마 직접 삭제 안 함 — kasina 옵션만 재고0 표시.
                #   kasina만 품절/삭제이어도 다른 몰 있으면 winner 이동, 없으면 reconcile 이 retire.
                add_log(f"  → 수집처 삭제/종료 → kasina 재고0 표시 (BUYMA 반영은 reconcile)")
                if not dry_run:
                    self._mark_all_out_of_stock(product['id'])
                    self.update_sync_time_only(product['id'])
                else:
                    add_log(f"  [DRY-RUN] kasina 재고0 표시 예정")
                with stats_lock:
                    stats['skipped'] += 1
...
def main():
    parser = argparse.ArgumentParser(description='바이마 재고/가격 동기화 (kasina)')

...
MEASUREMENT_KEY_CACHE_PATH = os.path.join(OKMALL_DIR, 'measurement_key_cache.json')

# 측정 키 리포트 저장 디렉토리
MEASUREMENT_REPORT_DIR = os.path.join(OKMALL_DIR, 'logs')
...
class RawToAceConverter:
    ...
    def load_color_master_id_mapping(self) -> Dict[str, int]:
        ...
        # colors.csv 파일 경로
        colors_csv_path = os.path.join(OKMALL_DIR, 'colors.csv')
            ...
            with self.engine.connect() as conn:
                ...
                if not existing:
                    conn.execute(text("""
                        ...
                        VALUES (:source_site, :path, :gender, :d1, :d2, :d3, :path, NULL, NULL)
                    """), {
                        'source_site': self.source_site,
    ...
    def fetch_raw_data(self, limit: int = None, brand: str = None, raw_id: int = None, upsert: bool = False, include_unpublished: bool = False) -> List[Dict]:
        with self.engine.connect() as conn:
            query = """
        ...
        with self.engine.connect() as conn:
            ...
            if row:
                return {
                    ...
                    'is_published': row[3],
                    # ON(단일권위): 이 ace 의 listing 이 바이마 등록됐나
    ...
    def save_ace_data(self, ace_data: Dict) -> int:
        with self.engine.connect() as conn:
            ...
            # kasina 자체 이미지: ace_product_images에 INSERT
            source_images = ace_data.get('source_images', [])
            for idx, img_url in enumerate(source_images):
                url = img_url if img_url.startswith('http') else f'https:{img_url}'
                conn.execute(text("""
                ...
                """), {'ace_product_id': ace_product_id, 'position': idx + 1, 'url': url})
            if source_images:
                log(f"  → 자체 이미지 {len(source_images)}장 저장 (source: kasina)")
    ...
    def run_conversion(self, limit: int = None, brand: str = None, dry_run: bool = False, raw_id: int = None, upsert: bool = False, skip_translation: bool = False, include_unpublished: bool = False) -> Dict:
        ...
        raw_data_list = self.fetch_raw_data(limit=limit, brand=brand, raw_id=raw_id, upsert=upsert, include_unpublished=include_unpublished)
...
def main():
    ...
    parser.add_argument('--include-unpublished', action='store_true',
                        help='미등록(is_published=0) 상품 전부 재변환 (raw.updated_at 비교 X) — --skip-collect 용')
    ...
    try:
        ...
        result = converter.run_conversion(limit=args.limit, brand=args.brand, dry_run=args.dry_run, raw_id=args.raw_id, upsert=args.upsert, skip_translation=args.skip_translation, include_unpublished=args.include_unpublished)

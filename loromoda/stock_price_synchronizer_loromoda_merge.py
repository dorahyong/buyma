# -*- coding: utf-8 -*-
"""
재고 및 가격 동기화 스크립트 (로로모다 버전)

바이마에 등록된 상품의 재고와 가격을 로로모다(loromoda.com)에서 재수집하여
ace 테이블을 업데이트하고 바이마 API로 상품을 수정합니다.

처리 흐름:
1. ace_products에서 바이마 등록 상품 조회 (is_published=1, buyma_product_id 있음)
2. 로로모다 상세 페이지 재방문 → 현재 가격/재고 수집
...
사용법:
    python stock_price_synchronizer_loromoda_merge.py                    # 전체 실행
    python stock_price_synchronizer_loromoda_merge.py --brand BURBERRY   # 특정 브랜드만
    python stock_price_synchronizer_loromoda_merge.py --limit 100        # 최대 100개만
    python stock_price_synchronizer_loromoda_merge.py --dry-run          # 테스트 (API 호출 안함)
    python stock_price_synchronizer_loromoda_merge.py --force            # 변경 없어도 강제 API 호출
...
# 딜레이 설정
REQUEST_DELAY_MIN = 1.0   # 로로모다 요청 간 최소 딜레이
REQUEST_DELAY_MAX = 2.0   # 로로모다 요청 간 최대 딜레이
...
class StockPriceSynchronizer:
    ...
    # -------------------------------------------------
    # 2. 로로모다에서 가격/재고 수집 (HTML 스크래핑)
    # -------------------------------------------------
    def collect_from_loromoda(self, product_url: str) -> Tuple[Dict, Optional[str]]:
        """
        로로모다(loromoda) 상세 페이지에서 가격/재고 수집 (loromoda 수집기와 동일 방식)

        - 가격: product_price JS변수(판매가) → 없으면 #span_product_price_text,
          정가는 #span_product_price_custom strike
        - 재고: option_stock_data JS 변수. 키(P0000 옵션코드)는 DB source_option_code 와 일치 확인됨(2/2).
          재고판정 = is_selling=='T' and stock_number>0 (★ loromoda 수집기와 동일하게 is_display 미체크).
        """
        ...
        try:
            ...
            # --- 가격 추출 (loromoda: product_price JS변수=판매가, 없으면 #span_product_price_text) ---
            pv = re.search(r"var\s+product_price\s*=\s*'?(\d+)'?", html)
            sale_price = int(pv.group(1)) if pv else None
            if not sale_price:
                sale_elem = soup.select_one('#span_product_price_text')
            ...
            # --- 옵션/재고: option_stock_data (loromoda 수집기와 동일: is_selling + stock_number>0) ---
            #   ★ loromoda는 option_stock_data 키(P0000…)가 DB source_option_code 와 일치 확인됨(2/2) →
            #     option_code 채워 1순위 코드 매칭. (혹시 stale이어도 detect_stock_changes 2순위 사이즈가 보완)
            #   ★ is_display 는 체크하지 않음(loromoda 수집기 정책과 동일).
            stock_data = {}
    ...
    def _mark_all_out_of_stock(self, ace_product_id: int) -> None:
        """loromoda 품절/삭제/흠집 → 이 ace 의 옵션 전부 out_of_stock 표시.
    ...
    def _reconcile_published(self, products: List[Dict]) -> None:
        ...
        ★ 이번 synced 상품에 한정 (loromoda published 전체 아님) → --limit/--id 테스트 안전.
...
※仕入れはデパートや公式オンラインショップなど、100％正規品のみ扱っております"""
    ...
    def process_single_product(self, product: Dict, idx: int, total: int, dry_run: bool, force: bool,
        ...
        try:
            # 1. 로로모다에서 가격/재고 수집
            mall_data, error = self.collect_from_loromoda(product['source_product_url'])
            if error:
                add_log(f"  로로모다 수집 실패: {error}", "WARNING")
                ...
                # [MERGE] 바이마 직접 삭제 안 함 — loromoda 옵션만 재고0 표시.
                #   loromoda만 품절/삭제이어도 다른 몰 있으면 winner 이동, 없으면 reconcile 이 retire.
                add_log(f"  → 수집처 삭제/종료 → loromoda 재고0 표시 (BUYMA 반영은 reconcile)")
                if not dry_run:
                    self._mark_all_out_of_stock(product['id'])
                    self.update_sync_time_only(product['id'])
                else:
                    add_log(f"  [DRY-RUN] loromoda 재고0 표시 예정")
                with stats_lock:
                    stats['skipped'] += 1
...
def main():
    parser = argparse.ArgumentParser(description='바이마 재고/가격 동기화 (loromoda)')
